...
- Per domande di orientamento sul codice ("dove sta X", "come funziona Y") usa il subagent `codebase-explorer` (Claude Code: `.claude/agents/`, Codex: `.codex/agents/`): è di sola lettura, non usarlo per fare modifiche.

## Configurazione agenti

- `AGENTS.md` (questo file) è la sorgente di verità per tutti i tool AI. `CLAUDE.md` è un symlink a `AGENTS.md`: modifica solo `AGENTS.md`.
- Le skill condivise vanno in `.agents/skills/<nome>/SKILL.md` (vedi `.agents/skills/README.md`). Codex CLI le legge nativamente, Claude Code tramite il symlink `.claude/skills -> ../.agents/skills`.
- Gli agent custom hanno formati diversi e vanno definiti in entrambi: `.claude/agents/<nome>.md` (Claude Code) e `.codex/agents/<nome>.toml` (Codex CLI), con lo stesso corpo di istruzioni.
- Su Windows i symlink richiedono `git config core.symlinks true` prima del clone; altrimenti `CLAUDE.md` appare come file di testo contenente "AGENTS.md".
